"""
exp1_clean.py
Experiment 1: Clean baseline - no attack.
All clients are honest. Establishes performance ceiling.
"""
...
def run_experiment(algorithm_name, aggregator, run_id):
    ...
    for rnd in range(N_ROUNDS):
        ...
        if (rnd + 1) % 10 == 0:
            print(f"    Round {rnd+1:2d} | Acc: {metrics['accuracy']:.4f} | F1: {metrics['f1']:.4f}")
...
def main():
    print("=" * 60)
    print("Experiment 1: Clean Baseline (No Attack)")
    ...
    for alg_name, aggregator in ALGORITHMS.items():
        ...
        for run_id in range(N_RUNS):
            round_metrics = run_experiment(alg_name, aggregator, run_id)
    ...
    for alg, res in all_results.items():
        print(f"{alg:<18} Acc: {res['accuracy_mean']:.4f}+/-{res['accuracy_std']:.4f}  F1: {res['f1_mean']:.4f}+/-{res['f1_std']:.4f}")
    ...
    fig.suptitle("Experiment 1: Clean Baseline (No Attack)", fontsize=13)
    metrics_keys = ["accuracy_mean", "f1_mean"]
    err_keys = ["accuracy_std", "f1_std"]
    ...
    plt.tight_layout()
    plot_path = os.path.join(RESULTS_DIR, "exp1_plot.png")
